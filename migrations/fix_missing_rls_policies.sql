...
-- 2. Clean up ALL existing policies on these tables to prevent conflicts
DO $$ 
DECLARE 
    r record;
BEGIN
...
-- 3. PROFILES: Allow everyone to read, but only owner or admin to modify
...
-- 4. PROJECTS: Allow business owner, assigned vendor, or admin to read/write
...
-- 5. PROJECT_ROUTING: Crucial fix for "Invite to Project"
...
-- 6. VENDOR_RESPONSES: Bids management
...
-- 8. METADATA: Read-only access for all authenticated users
...
-- 9. FINAL GRANTS (Ensures the database roles have permission to execute the queries)
